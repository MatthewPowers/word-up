...
function addNewWordSubmission(word) {
    ...
    {
        wordlist.push(model.wordSubmissions[x].word);
    }
    ...
    $.ajax({
        ...
        success: function(response) {
           ...
           var words = model.wordSubmissions.map(function(submissions) 
           {
            ...
            {
                submissions.isRealWord = isRealWord;
            }
            });
...
function wordSubmissionChip(wordSubmission) {
    ...
    if (wordSubmission.hasOwnProperty("isRealWord")) 
    {
        ...
        {
            //console.log("is true");
            var newword = wordSubmission.word
            var score = wordScore(newword);
            //console.log(score);
            //scoreChip.append(score);
            var scoreChip = $("<span></span>").text(score).css({"background-color": "blue", "font-size": "50%", "margin-left": "6px"});
            //$("wordChip").css({"background-color": "lightblue", "font-size": "85%"});
        }
        else
        {
            ...
            score = "X"
            //scoreChip.append(score);
            scoreChip = $("<span></span>").text("X").css({"background-color": "red", "font-size": "50%", "margin-left": "6px"});
            //$("wordChip").css({"background-color": "lightred", "font-size": "85%"});
            //console.log(score);
        }
        //wordScore
...
function wordScore(word) {
    ...
    {
        var wordScores = letterScore(letters[i]);
        letterScores.push(wordScores);
    }
